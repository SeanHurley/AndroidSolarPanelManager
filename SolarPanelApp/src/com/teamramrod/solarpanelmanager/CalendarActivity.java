 ...
 * scheduled events and provides a functionality to delete existing events and
 * add new ones.
 * 
 * @author michael
 */
public class CalendarActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// this legwork should probably be handled in a shared SolarActivity
		// superclass
		...
		arrayAdapter = new ArrayAdapter<Event>(this, android.R.layout.simple_list_item_1);
	...
	/**
	 * Make a request to the controller to remove an event. If successful, also
	 * remove the event from the interface.
	 * 
	 * @param event
	 *            the event to delete
	 */
	...
	/**
	 * Make a request to the controller to add an event. If successful, add the
	 * event to the interface. The id of the event to be added doesn't matter
	 * since the controller will assign an id.
	 * 
	 * @param event
	 *            the event to add
	 */
	private void addEvent(final Event event) {
		...
		(new ScheduleEventHandler(new Callback<BaseResponse>() {
			...
			public void onComplete(BaseResponse response) {
				...
				if (response == null) {
					...
					// TODO: replace event data with data from the message
					// instead of
					// assuming it's the same as the event requested to be
					// added.
					Event toAdd = new Event(id, event.getName(), event.getFirstTime(), event.getDuration(),
							event.getInterval());
					if (!calendar.addEvent(toAdd)) {
						Toast.makeText(CalendarActivity.this, "Event conflicts with another event", Toast.LENGTH_SHORT)
								.show();
					} else {
		...
		}, deviceId, pass, event.getName(), event.getFirstTime(), event.getDuration(), event.getInterval()))
				.performAction();
	}
	...
	private class EventClickListener implements OnItemClickListener {
		...
		@Override
		public void onItemClick(final AdapterView<?> lis, View arg1, final int position, long arg3) {
